        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("hikka.inline", "custom_bot", False):
                ...
                try:
                    ...
                    uid = utils.rand(6)
                    username = f"@SCIENSE_{uid}_bot"
            else:
                uid = utils.rand(6)
                username = f"@SCIENSE_{uid}_bot"

            for msg in [
                f"🪐 SCIENSE userbot"[:64],
                username,
                "/setuserpic",
                username,
            ]:
            ...
            try:
                ...
                m = await conv.send_file(main.BASE_PATH / "assets" / "SCIENSE-ava.png")
                r = await conv.get_response()
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@SCIENSE_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    try:
                        ...
                        m = await conv.send_file(
                            main.BASE_PATH / "assets" / "SCIENSE-ava.png"
                        )
                        r = await conv.get_response()
