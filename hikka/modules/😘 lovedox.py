...
@loader.tds
class love(loader.Module):
    """love докс модуль 🥰️"""
    strings = {'name':'😘 lovedox'}

    @loader.command(alias='лове')
    async def love(self, message):
        """[реплай] - пробить человека на любовь разроботчик @Python_Javs"""
        love_msg = [
            "🩶 <b>запрашываю Infomasion...</b>", 
            "🩶 <b>фыйлы найдены, п0лучаю инф0рмацию...</b>", 
            "🩶 <b>озникла ошыбка перезаписуваю файлы</b>"
            ]
        love = random.choice(love_msg)
        await utils.answer(message, love)
        await sleep(2)
        zapros_msg = [
            "🩷 <b> обрабляем </b> <code>[ 3% ] </code>",
            "❤️‍🔥 <b>исправляю ошыбки и нахожу файлы"
            ]
        ...
        await sleep(1)
        await utils.answer(message, "💛 <b>обрабатуем запрос..</b> <code> [ 13% ]</code>")
        await sleep(1)
        ...
        await sleep(1)
        await utils.answer(message, "❤️‍🩹 <b>Инф0рмᴀция нᴀйдᴇʜᴀ</b> <code> [ 94% ]</code>")
        await sleep(0.6)
        ...
        countries = ["милосьти", "во сне", "нету", "жыву как хочу"]
        regions = ["твой сон", "радость", "улыбка"]
        operators = ["хз", "милость", "смерть", "тот свет", "цердце", "любовь", "душа"]
        names = ["сон", "конфетка", "незнакомка", "хз", "любовница", "милашка", "королева"]
        addresses = ["боль", "душа","сон", "Ничего больше не придумала"]
        birth_dates = ["14.01.2005 (19 лет)", "16.06.6666 (ченахуй)", "14.08.8 (пасхалко)", "сегодня"]
        ...
        social_marks = ["🙄 Ноунеймка (0)", "🤔 Известная (1)", "🧸 мишка! (2)", "🎀️ подарок (3)", "🧷 хз (4)", "🙄 Ноунеймка (5)", "💎 𝐕𝐈𝐏 (6)", "🇷🇺 Богиня (7)", "🙄 Ноунеймка (8)", "🙄 Ноунеймка (9)", "🔰 Сильнейшия (10)", "🙄 Ноунеймка (11)", "🙄 Ноунеймка (12)", "🙄 Ноунеймка (13)","🙄 Ноунеймка (14)", "🙄 Ноунеймка (15)"]
        ...
        reputation = f"({random.choice(reputations_positive)})🩵 ({random.choice(reputations_negative)})❤️‍🩹"
        social_mark = random.choice(social_marks)
        comments_count = random.choice(comments_counts)

        result_message = (
            f"❤️‍🔥<b>сколько влюблены в зайку</b> <code>{random_number}</code>\n"
            f"<b>💖 страна  солнешки:</b> <code>{random.choice(countries)}</code>\n"
            f"<b>🩶 Регион солнушки:</b> <code>{random.choice(regions)}</code>\n"
            f"<b>❤️‍🔥 место:</b> <i>{random.choice(operators)}</i>\n\n"
            f"💞 <b>никнеймы зайки:\n└</b>  <i>{random.choice(names)}</i>\n\n"
            f"🧸 <b>секрет:</b> \n<i>{random.choice(addresses)}</i>\n\n"
            f"🥰 <b>Дата рождения свое:</b> <code>{random_birth_date}</code>\n\n"
            f"💙 <b>влюблены :</b> <code>{viewers_count}</code> человек\n"
            f"🩵 <b>любовь:</b> <i>{reputation}</i>\n"
            f"🎧 <b>Социальная метка зайки:</b> <code>{social_mark}</code>\n"
            f"🥳 <b>подарков:</b> <code>{comments_count}</code>"
        )
