...
@loader.tds
class SCIENSESettingsMod(loader.Module):
    """Advanced settings for SCIENSE Userbot"""

    strings = {"name": "SCIENSESettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"SCIENSE-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "SCIENSE" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
